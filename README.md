...
`main` also calls `save_analytic_controls()`, which writes the analytic pulse itself in the
same 8-control format so step 3 can convert it too — see
[The analytic pulse takes the same path](#the-analytic-pulse-takes-the-same-path).
...
| | analytic | GRAPE, T_frac = 1 | GRAPE, T_frac = 0.5 |
|---|---|---|---|
| duration | 225.7 µs | 225.7 µs | 112.8 µs |
| fidelity | 0.999903 | 0.999903 | 0.992380 |
| peak amplitude | 23.9 / 17.0 of 100 | 23.9 / 17.0 of 100 | 32.9 / 30.3 of 100 |
| amplitude error | ≤ 0.003/100 | ≤ 0.003/100 | ≤ 0.003/100 |
| phase error | 0.0000° | 0.0000° | 0.0000° |
| bytecode | 480 words | 1520 words | 2264 words |
| | **PASS** | **PASS** | **PASS** |

### The analytic pulse takes the same path

The analytic protocol is just a particular point in the same 8-control space, so it needs
no separate export path. `save_analytic_controls` (in `spinboson_grape.jl`, called by
`main`) discretizes the guess onto the time grid and writes it with the *identical* JLD2
schema, and `export_jaqalpaw.jl` picks it up along with the GRAPE files:

```bash
julia --project=. -e 'include("spinboson_grape.jl"); save_analytic_controls()'
julia --project=. export_jaqalpaw.jl        # converts all three, one figure each
```

giving `results/jaqalpaw_tones_analytic_Tfrac100.png` next to the two GRAPE ones. It is
cheap — no optimization — so this, not a GRAPE run, is the way to get the analytic pulse
onto hardware.

The figure is the clearest picture of what the protocol actually asks the hardware to do:
ion 1's two sidebands trade amplitude back and forth at |Δ| between 0 and √2·g₀ = 9.95 kHz
with phases stepping between four fixed values, then go dark at `t_strobo = 112.8 µs`,
at which point ion 2 turns on flat at √2·g₀ = 7.07 kHz — red at +135°, blue at −45° —
for the displacement stage.

Two things to expect when comparing it with the `T_frac = 1` GRAPE export. They agree to
`< 1e-4°` in phase and `< 3 mHz` in rate wherever the drive is on, because at full
duration GRAPE returns the guess untouched; the phase arrays nevertheless differ by up to
164° on the half of the samples where the tone is idle, since the phase of a zero-amplitude
tone is arbitrary. And the analytic pulse compiles to **480 bytecode words against GRAPE's
1520** for the same 250 samples and the same duration — it is piecewise constant with only
a handful of distinct amplitude/phase pairs, so most of its modulation steps compress away.
...
results/
  ...
  spinboson_analytic_controls_Tfrac100.jld2      the analytic pulse, same schema
  *_jaqalpaw.json                                regenerated by export_jaqalpaw.jl
  jaqalpaw_tones_{analytic_Tfrac100,grape_Tfrac100,grape_Tfrac50}.png
