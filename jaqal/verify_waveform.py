"""End-to-end check of the exported spin-dependent squeezing pulse.
...
from spinboson_pulses import SpinBosonPulses, load_drive  # noqa: E402

JAQAL_FILE = os.path.join(HERE, "spinboson_analytic.jaqal")
...
def check_tone(gp, drive, tone, amp_tol=0.05, phase_tol=0.5, dump=None):
    """Compile and emulate the gate with only `tone` modulated, and compare.
    ...
    code = CIRCUIT.format(channel=gp.target0, tone_mask=1 << tone)
    with temp_circuit(code) as path:
        record = emulate(compile_bytecode(path))
    ch = record[gp.target0]

    t_upd, amp = find_gate_updates(ch[MOD[f"a{tone}"]], n, step_cycles)
    _, phase = find_gate_updates(ch[MOD[f"p{tone}"]], n, step_cycles)

    want_amp = np.asarray(gp.sb_rate_to_amp(ion[f"{side}_rate_hz"]))
    ...
    print(f"  tone {tone} ({side:4s}): amp err {amp_err:7.4f} (tol {amp_tol}), "
...
def main(argv=None):
    ...
    gp = SpinBosonPulses()
    drive = load_drive(gp.sb_pulse_file)
    n = drive["n_samples"]
    ...
    ok = all(check_tone(gp, drive, tone, dump=dump) for tone in (0, 1))

    print("\n=== full two-tone gate ===")
    code = compile_bytecode(JAQAL_FILE)
    n_words = len(code) // 32
    print(f"  {JAQAL_FILE.split(os.sep)[-1]} compiles: "
